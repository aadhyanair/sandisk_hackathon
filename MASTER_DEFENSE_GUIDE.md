...
- **Panel 1 (Pre-Test / `old_label`):** Shows the wafer at Probe 1. The **Emerald Green (`#10b981`)** background indicates surviving silicon; **Deep Crimson Red (`#b91c1c`)** dots indicate dies that failed inline electrical checks.
- **Panel 2 (Post-Test / `label`):** Shows the wafer after dicing, packaging, and high-voltage burn-in. More red dots have appeared.
- **Panel 3 (Difference Map):** The ground truth comparison with high-contrast semantic separation:
  - **Emerald Green (`#10b981`) dies:** Survived both tests.
  - **Deep Crimson Red (`#b91c1c`) dies:** Pre-existing gross failures discarded at Probe 1 (low luminance, dark burgundy tone).
  - **Radiant Electric Orange (`#ff7a00`) dies:** **NEW LATENT FAILURES.** Dies that passed Probe 1 as "good" but died during package burn-in. Engineered with a $+30\%$ luminance gap and $30^\circ$ hue shift from deep crimson red, ensuring zero visual ambiguity.
- **Prediction Outcome Companion View:**
  - **TP (Caught New Fail):** Radiant Electric Orange (`#ff7a00`).
  - **FN (Missed New Fail):** Vivid Signal Red (`#ef4444`).
  - **FP (False Alarm / Overkill):** **Electric Sky Blue / Azure (`#0284c7`)** (Replaced dull purple with clinical, high-contrast cyan blue).
  - **TN (Passing Die):** Clean Light Slate (`#e2e8f0`).
